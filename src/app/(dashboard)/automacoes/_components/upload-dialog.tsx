...
export function AutomacaoUploadDialog({ open, onClose }: Props) {
  ...
  async function handleUpload() {
    ...
    const res = await fetch("/api/automacoes/upload", { method: "POST", body: formData });
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Upload size={16} /> Importar automações via Excel
          </DialogTitle>
        </DialogHeader>
          ...
          {result && (
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-sm text-green-600 font-medium">
                ...
                {result.inserted} automação{result.inserted !== 1 ? "ões" : ""} importada{result.inserted !== 1 ? "s" : ""} com sucesso
              </div>
